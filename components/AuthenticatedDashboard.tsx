...
export function AuthenticatedDashboard() {
  ...
  useEffect(() => {
    ...
    // Cleanup function correctly returns void
    return () => {
      tl.kill()
    }
  }, [])
        ...
        <div className="max-w-5xl mx-auto space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {models.map((model, index) => (
              <Link key={model.title} href={model.href} className="block group">
                <Card 
                  ref={el => cardsRef.current[index] = el}
                  className="transition-all duration-300 hover:shadow-md hover:border-primary transform hover:-translate-y-1 rounded-lg border border-border"
                >
